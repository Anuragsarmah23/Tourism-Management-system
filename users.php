	...
	<body style="background:url('img/c4.jpg');">
		...
		<div class="sidebar">
			<ul class="sidebar-nav">
				...
				<li class="side-item">
					<a href="users.php" class="side-link active"><i class="fa fa-users"></i><span class="ml-3">VIEW USERS</span></a>
				</li><hr style="border-color:grey;">
				<li class="side-item">
		...
		<div style="margin-left:16vw;padding-top:78px;margin-right:1vw;">
			...
			<div class="row bg-transparent text-light mx-2 p-3" style="border:1px solid black;">
				...
				<span class="ml-auto" style="font-size:16px;"><a href="register.php">ADD <i class="fa fa-plus"></i></a></span>
			</div><br>
			<?php
				//Radhe
				$con=mysqli_connect("localhost","root","","tams");
				...
				<table id="userList" class="table table-striped table-bordered bg-transparent text-light" style="width:100%">
					...
					<?php
						while($row=mysqli_fetch_assoc($q_run)){
							$years = round((time()-strtotime($row['dob']))/(3600*24*365.25))
					?>
						<tr>
							...
							<td><a href="editUser.php?id=<?php echo $row['id']; ?>"><i class="fa fa-edit editI"></i></a></td>
							<td><a href="delUser.php?id=<?php echo $row['id']; ?>"><i class="fa fa-trash text-danger delI"></i></a></td>
						</tr>	
					<?php
						}
					?>	
